...
To install the tool, please run this command in your terminal (on Windows use the Command Prompt, not the Python console):

```bash
pip3 install bacolonyzer
```

Or on newer versions (Python 3.9)
```bash
py -m pip install bacolonyzer
```
...
If the top box does not have a variable named "path" click New to create one, set the name to "Path", and the value to your desired path, e.g. "C:\Program Files (x86)\Python\Scripts".
